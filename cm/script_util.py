...
from .karras_diffusion import KarrasDenoiser, OneShotDenoiser, EDMEDMDenoiser
...
def create_one_shot_edmedm_model_and_diffusion(
    image_size,
    class_cond,
    learn_sigma,
    num_channels,
    ...
    sigma_max=80.0,
    distillation=False,
    teacher_precond=False,
    loss_norm="l2",
    random_init=False,
    continuous=False,
):  
    from cm.network import DhariwalUNet, SongUNet
    channel_mult__ = channel_mult
    ...
    if channel_mult__ == "":
        ...
        channel_mult = tuple(int(ch_mult) for ch_mult in channel_mult__.split(","))
    ...
    if image_size == 32:
        model = SongUNet(
            ...
            dropout=dropout,
            channel_mult=channel_mult,
            num_classes=(10 if class_cond else None),
            use_checkpoint=use_checkpoint,
            use_fp16=use_fp16,
            num_heads=num_heads,
            num_head_channels=num_head_channels,

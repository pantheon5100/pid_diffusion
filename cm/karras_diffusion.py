...
class OneShotDenoiser:
    ...
    def idx2sigmas(self, indices, num_scales):
    ...
    def sample_heun(
        ...
        sigmas = get_sigmas_karras(steps, self.sigma_min, self.sigma_max, self.rho, device=x.device)
        indices = range(len(sigmas) - 1)

        target_trajectory = th.zeros_like(x)
        _, c, h, w = x.size()
        max_sigmas_target = sigmas_target.max().item()

        position_record = th.arange(0, x.shape[0], device=x.device).long()
        ...
        for i in indices:
            ...
            # remove the reached sample
            mask = sigmas_target == i

            if mask.sum() > 0:
                reached_sample = position_record[mask]
                target_trajectory[reached_sample] = x[mask]
                if i == max_sigmas_target:
                    break
                # redefine the x and sigmas
                x = x[~mask].view(-1,c,h,w)
                sigmas_target = sigmas_target[~mask]
                position_record = position_record[~mask]
    ...
    def ode_losses(
        ...
        if model_kwargs is None:
            ...
            model_kwargs = {"y": model_kwargs['noise_cond']}

        dims = x_start['noise'].ndim
        ...
        t = self.idx2sigmas(indices, num_scales)
        
        indices_plus1 = (indices + 1)
        indices_plus1[indices_plus1==num_scales] = num_scales-2
        t2 = self.idx2sigmas(indices_plus1, num_scales)

        y_hat = denoise_fn(x_start, t)
        y_hat_dt = denoise_fn(x_start, t2)
        dydx = (y_hat-y_hat_dt)/append_dims(t - t2, dims)
    ...
    def denoise(self, model, noise, sigmas, old_time=False, **model_kwargs):
        if not isinstance(sigmas, list):
            sigmas = sigmas
            c_noise = 0.25 * th.log(sigmas)*self.t_rescale
            
            sigma_max = self.sigma_max
        else:
            sigmas_init_cond = sigmas[1]
            sigmas = sigmas[0]

            c_noise = [0.25 * th.log(sigmas), 0.25 * th.log(sigmas_init_cond)]
            sigma_max = sigmas_init_cond.view(-1, 1, 1, 1)

        c_skip, c_out, c_in = [
            append_dims(x, noise.ndim) for x in self.get_scalings(sigmas)
        ]
        c_in = 1 / math.sqrt(self.sigma_data ** 2 + 80.0 ** 2)
        F_x = model((c_in * noise), c_noise, **model_kwargs)
        output = c_out * F_x + c_skip * noise

        init_weight = sigmas.view(-1, 1, 1, 1) / sigma_max
        model_weight = 1-init_weight

        x_t = init_weight * noise + model_weight * F_x
...
@th.no_grad()
def ode_solver(teacher_denoise_fn, x_ti, t, t_ip1,type='EulerHeun'):
    dims = x_ti.ndim
    if type == 'EulerHeun':
        denoised = teacher_denoise_fn(x_ti, t)

        # Heun's method
        dydx = (x_ti-denoised)/append_dims(t, dims)
        dt = t_ip1 - t
        x_2 = x_ti + dydx * append_dims(dt, dims)

        denoised_2 = teacher_denoise_fn(x_2, t_ip1)

        dydx_2 = (x_2-denoised_2)/append_dims(t_ip1, dims)  
        d_prime = (dydx + dydx_2) / 2
        x = x_ti + d_prime * append_dims(dt, dims)
        ...
        dydx = (x_ti-denoised)/append_dims(t, dims)
        dt = t_ip1 - t
        x = x_ti + dydx * append_dims(dt, dims)
...
class EDMEDMDenoiser:
    ...
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        ...
        #     ]
        rescaled_t = 0.25 * th.log(sigmas)*self.t_rescale
...
def karras_sample(
    ...
    sample_fn = {
        "heun": sample_heun_deter,
        "heun_deter": sample_heun_deter,
        "dpm": sample_dpm,
...
def sample_oneshot(
    ...
    """Single-step generation from a distilled model."""
    # s_in = x.new_ones([x.shape[0]])
    sigmas = th.tensor([0.002]).view(-1,).to(x.device).repeat(x.shape[0],)
    return distiller(x, sigmas)
